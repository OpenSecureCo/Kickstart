...
rollBack() {
    ...
    if [ -z "${uninstall}" ]; then    
        logger -w "Installation cleaned. Check the /var/log/opensecure-installation.log file to learn more about the issue."
    fi
...
installWazuh() {
    ...
    if [ ${sys_type} == "zypper" ]; then
        ...
        eval "wget https://raw.githubusercontent.com/OpenSecureCo/Kickstart/main/custom-ssh.sh -O /var/ossec/active-response/bin/custom-ssh.sh ${debug}"
        eval "wget https://raw.githubusercontent.com/OpenSecureCo/Kickstart/main/custom-suricata.sh -O /var/ossec/active-response/bin/custom-suricata.sh ${debug}"
        eval "wget https://raw.githubusercontent.com/OpenSecureCo/Kickstart/main/custom-waf.sh -O /var/ossec/active-response/bin/custom-waf.sh ${debug}"
        eval "chown root:ossec /var/ossec/active-response/bin/* ${debug}"
        eval "chmod 750 /var/ossec/active-response/bin/* ${debug}"
...
installYara() {
    ...
    if [ ${sys_type} == "yum" ]; then
        ...
        eval "cd /opt/yara-4.1.3/ ${debug}"
        eval "./bootstrap.sh ${debug}"
        eval "./configure --enable-cuckoo --enable-magic --enable-dotnet ${debug}"
        eval "make ${debug}"
        eval "make install ${debug}"
        eval "cd /usr/local/ ${debug}"
        eval "git clone https://github.com/Neo23x0/signature-base.git ${debug}"
        eval "mkdir /root/scripts/ ${debug}"
        eval "wget https://raw.githubusercontent.com/OpenSecureCo/Kickstart/main/$CUSTOMER/yara_update_rules.sh -O /root/scripts/yara_update_rules.sh ${debug}"
        eval "wget https://raw.githubusercontent.com/OpenSecureCo/Kickstart/main/$CUSTOMER/yara_full_scan.sh -O /var/ossec/active-response/bin/yara_full_scan.sh ${debug}"
        eval "chmod +x /root/scripts/yara_update_rules.sh ${debug}"
...
/usr/sbin/' > /opt/scanfolders.txt ${debug}"
        eval "wget https://raw.githubusercontent.com/OpenSecureCo/Kickstart/main/$CUSTOMER/Freshclam.conf -O /etc/freshclam.conf ${debug}"
        eval "wget https://raw.githubusercontent.com/OpenSecureCo/Kickstart/main/$CUSTOMER/scan.conf -O /etc/clamd.d/scan.conf ${debug}"
        eval "mkdir /root/scripts/ ${debug}"
...
networkCheck() {
    connectionReg=$(sleep 2 | telnet $MANAGER $WAZUHPORTREG 2>/dev/null | grep Connected | awk '{print $1}')
    ...
    connectionLog=$(sleep 2 | telnet $MANAGER $WAZUHPORTLOG 2>/dev/null | grep Connected | awk '{print $1}')
...
main() {
    ...
    checkArch
    touch /var/log/opensecure-installation.log
    ...
    if [ -n "$1" ]; then      
        ...
        if [ -n "${verbose}" ]; then
            debug='2>&1 | tee -a /var/log/opensecure-installation.log'
        fi
